...
private:
    ...
    std::map<std::string, uint16_t> ipport_map;
...
bool Scanner::callback(PDU &pdu)
{
    ...
    if (target_addresses.contains(ip.src_addr()) && target_ports.count(tcp.sport()) == 1)
    {
        ...
        {
            char a[256];
            sprintf(a, "http://%s:%d", ip_address.c_str(), tcp.sport());
            std::string astr = a;

            std::map<std::string, uint16_t>::iterator mit;
            mit = ipport_map.find(ip_address);
            if (mit == ipport_map.end())
            {
                if (reqHead(astr) == EXIT_SUCCESS)
                {
                    ipport_map.insert(std::pair<std::string, uint16_t>(ip_address, tcp.sport()));
                }
            }
            // std::cout << ip_address << " (" << ip_to_host(ip_address.c_str()) << ")\t\tPort: " << tcp.sport() << " http open\n";
